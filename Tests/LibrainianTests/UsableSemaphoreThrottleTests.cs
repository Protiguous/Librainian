...
public class UsableSemaphoreThrottleTests {
	...
	private static async Task<TimeSpan> DoThings( IUsableSemaphoreThrottle throttle, TimeSpan taskTimeSpan ) {
		if ( throttle is null ) {
	...
	public async Task WaitSemaphoreThrottleAsync( Int32 semaphoreMilliseconds, Int32 taskMilliseconds ) {
		...
		Assert.That( await taskA.ConfigureAwait( false ) > time, Is.True );
		Assert.That( await taskA.ConfigureAwait( false ) < await taskB.ConfigureAwait( false ), Is.True );

		Assert.That( await taskB.ConfigureAwait( false ) > time + semaphoreTimeSpan, Is.True );
		Assert.That( await taskB.ConfigureAwait( false ) < await taskC.ConfigureAwait( false ), Is.True );
	}
}