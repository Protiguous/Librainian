...
// 
// File "ConcurrentListTests.cs" last touched on 2022-02-22 at 7:10 AM by Protiguous.
...
public static class ConcurrentListTests {
	...
	/// <summary>
	///     Environment.ProcessorCount * 10240 = 163840
	/// </summary>
	public static Int32 Threads { get; } = Environment.ProcessorCount * 1024;
	...
	private static (Int32 added, Int32 removed) AddManyProductsRemoveRandom( ICollection<String?> list ) {
		...
		Parallel.For( 0, added, x => {
			if ( Randem.NextBoolean() && list.Remove( $"name {x}" ) ) {
