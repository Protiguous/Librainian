...
//
// File "ConcurrentList.cs" last formatted on 2022-02-06 at 5:53 AM by Protiguous.
...
public class ConcurrentList<T> : ABetterClassDispose, IList<T?> /*, IEquatable<IEnumerable<T>>*/ {
	...
	public ConcurrentList<T> Clone() {
		ConcurrentList<T> copy = new( this.Read( () => this.TheList ) );
	...
	/// <summary>
	///     <para>
	///         Returns an enumerator that iterates through this <see cref="ConcurrentList{TType}" /> .
	///     </para>
	/// </summary>
	public IEnumerator<T> GetEnumerator() => this.Read( () => this.TheList.GetEnumerator() );
