...
//
// File "App.cs" last formatted on 2022-02-06 at 6:18 AM by Protiguous.
...
public static class App {
	...
	private static void CommonPreRun() {
		...
		try {
			...
			if ( highDpiMode ) {
				$"{nameof( Application.SetHighDpiMode )} has been set.".TraceWriteLine();
			}
    ...
    */
	...
	public static void Run<TForm>( this IHost host ) where TForm : Form {
		try {
			CommonPreRun();

			var form = ( host.Services ?? throw new NullException( nameof( host ) ) ).GetRequiredService<TForm>();

			form.PersistPlacement();

			if ( form.InvokeRequired ) {
				$"{nameof( form.InvokeRequired )} on {nameof( form.Name )}.".DebugWriteLine();
			}
