...
//
// File "D.cs" last formatted on 2022-12-22 at 5:20 PM by Protiguous.
...
public record D : IEqualityComparer<D> {
	public D() {
		this.K = String.Empty;
		this.V = null;
	}
	...
	public Int32 GetHashCode( D obj ) => obj.K.GetHashCode();
	...
	public static Boolean Equals( D? left, D? right ) {
		if ( left is null || right is null || left.V is null || right.V is null ) {
			return false;
		}

		return left.K.Equals( right.K, StringComparison.Ordinal ) && left.V.Equals( right.V, StringComparison.Ordinal );
	}
