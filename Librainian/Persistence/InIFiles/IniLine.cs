...
// 
// File "IniLine.cs" last formatted on 2022-02-14 at 6:18 AM by Protiguous.
...
public class IniLine {
    ...
    public IniLine( String? key, String? value = default ) {
        ...
        ( var status, var start ) = key.StartsWith( CommentHeaders, StringComparison.Ordinal );

        if ( status.IsGood() ) {
            this.Key = key;
            this.Value = value;
            this.LineType = LineTypes.Comment;
            this.LineHeader = start;
