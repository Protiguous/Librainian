...
//
// File "TheInternet.cs" last formatted on 2022-12-22 at 5:17 PM by Protiguous.
...
public static class TheInternet {
	...
	public static async Task<IDocumentFile?> DownloadAsync(
		Uri address,
		TimeSpan timeOut,
		IProgress<ZeroToOne>? reportProgress = null,
		ICredentials? credentials = null,
		Action<Uri, WebExceptionStatus>? onWebException = null
	) {
		if ( address is null ) {
		...
		return default( IDocumentFile );
	}

	public static IEnumerable<DocumentFile> FindFile( String filename, IEnumerable<String> locationClues ) {
		...
		foreach ( var locationClue in locationClues ) {
			...
			yield return new DocumentFile( internetAddress.ToString() ); //should download file to a document in the user's temp folder.
		}
	}
}