...
//
// File "AllSeeingFileWatcher.cs" last formatted on 2022-12-22 at 5:15 PM by Protiguous.
...
public class AllSeeingFileWatcher : ABetterClassDisposeAsync {
	...
	public AllSeeingFileWatcher( String? disposeHint ) : base( disposeHint ) {
	}
	...
	public override async ValueTask DisposeManagedAsync() => await this.Stop().ConfigureAwait( false );

	public async Task Start() {
		var drives = await Task.Run( DriveInfo.GetDrives ).ConfigureAwait( false );

		await foreach ( var drive in drives.ToAsyncEnumerable().ConfigureAwait( false ) ) {
	...
	public async Task Stop() {
		await foreach ( var watcher in this.FileWatchers.ToAsyncEnumerable().ConfigureAwait( false ) ) {
			using ( watcher ) { }
		}
	}
}