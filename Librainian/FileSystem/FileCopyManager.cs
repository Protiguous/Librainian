...
//
// File "FileCopyManager.cs" last formatted on 2022-12-22 at 5:15 PM by Protiguous.
...
public class FileCopyManager {
	private ConcurrentDictionary<DocumentFile, Task<FileCopyData>> SourceFilesToBeCopied { get; } = new();
	...
	public async Task LoadFilesToBeCopied(
		IAsyncEnumerable<DocumentFile> sourceFiles,
		...
		await foreach ( var sourceFile in sourceFiles.WithCancellation( cancellationToken ).ConfigureAwait( false ) ) {
			var destinationDocument = new DocumentFile( destinationFolder, sourceFile.FileName );
