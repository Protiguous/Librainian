...
//
// File "LazyAsync.cs" last formatted on 2022-02-06 at 5:39 AM by Protiguous.
...
public sealed class LazyAsync<T> {
	...
	public LazyAsync( Func<T> factory ) => this.instance = new Lazy<Task<T>>( () => Task.Run( factory ) );
	...
	///     needed.
	/// </param>
	public LazyAsync( Func<Task<T>> factory ) => this.instance = new Lazy<Task<T>>( () => Task.Run( factory ) );
	...
	/// </summary>
	public void Start() => this.instance.Value;
}
