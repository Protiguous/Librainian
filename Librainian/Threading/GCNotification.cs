...
//
// File "GCNotification.cs" last formatted on 2022-02-22 at 7:22 AM by Protiguous.
...
public static class GcNotification {
	...
	private sealed class GenObject {
		...
		~GenObject() {
			...
			// This is the Finalize method If this object is in the generation we want (or higher), notify the delegates that a GC just completed
			if ( GC.GetGeneration( this ) >= this._generation ) {
				var temp = Interlocked.CompareExchange( ref _sGcDone, null, null );
				temp?.Invoke( this._generation );
			}
		...
		public GenObject( Int32 generation ) => this._generation = generation;
	}
