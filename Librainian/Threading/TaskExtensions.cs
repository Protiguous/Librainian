...
// 
// File "TaskExtensions.cs" last formatted on 2022-02-22 at 7:27 AM by Protiguous.
...
public static class TaskExtensions {
    ...
    public static void Execute( this Action action, Object[]? args = default ) {
        foreach ( var method in action.GetInvocationList() ) {
            try {
                _ = method.Target is ISynchronizeInvoke {
                    InvokeRequired: true
                } syncInvoke ? syncInvoke.Invoke( method, args ) : method.Method.Invoke( method.Target, args );
            }
            catch ( Exception exception ) {
                exception.Log();
            }
    ...
    public static IEnumerable<Task<T>> InCompletionOrder<T>( this IEnumerable<Task<T>> source ) {
        ...
        var boxes = inputs.ConvertAll( _ => new TaskCompletionSource<T>( TaskCreationOptions.RunContinuationsAsynchronously ) );
        var currentIndex = -1;
