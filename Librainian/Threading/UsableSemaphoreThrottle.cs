...
// 
// File "UsableSemaphoreThrottle.cs" last formatted on 2022-02-22 at 7:18 AM by Protiguous.
...
public class UsableSemaphoreThrottle : IUsableSemaphoreThrottle {

    private readonly IUsableSemaphoreThrottle _semaphore;
    ...
    public UsableSemaphoreThrottle( IThrottle throttle, IUsableSemaphoreThrottle semaphore ) {
    ...
    public async Task<IUsableSemaphoreWrapper> WaitAsync() {
        var wrapper = default( IUsableSemaphoreWrapper? );
    ...
    public async Task<IUsableSemaphoreWrapper> WaitAsync( TimeSpan timeout ) {
        ...
        try {
            wrapper = await this._semaphore.WaitAsync( timeout ).ConfigureAwait( false );
            await this._throttle.WaitAsync().ConfigureAwait( false );
    ...
    public async Task<IUsableSemaphoreWrapper> WaitAsync( CancellationToken cancellationToken ) {
        ...
        try {
            wrapper = await this._semaphore.WaitAsync( cancellationToken ).ConfigureAwait( false );
            await this._throttle.WaitAsync().ConfigureAwait( false );
    ...
    public async Task<IUsableSemaphoreWrapper> WaitAsync( TimeSpan timeout, CancellationToken cancellationToken ) {
        ...
        try {
            wrapper = await this._semaphore.WaitAsync( timeout, cancellationToken ).ConfigureAwait( false );
            await this._throttle.WaitAsync().ConfigureAwait( false );
