...
//
// File "NetworkAdapter.cs" last formatted on 2022-12-22 at 5:14 PM by Protiguous.
...
public class NetworkAdapter {
	...
	public static IEnumerable<NetworkAdapter> GetAllNetworkAdapters() {
		var networkAdapters = "SELECT DeviceID, ProductName, NetEnabled, NetConnectionStatus FROM Win32_NetworkAdapter WHERE Manufacturer <> \'Microsoft\'".WmiQuery();
	...
	public async Task<EnumEnableDisableResult> EnableOrDisableNetworkAdapter( String strOperation, CancellationToken cancellationToken ) {
		...
		using ( crtNetworkAdapter = new ManagementObject() ) {
			try {
				var networkAdapters = $"SELECT DeviceID, ProductName, NetEnabled, NetConnectionStatus FROM Win32_NetworkAdapter WHERE DeviceID = {this.DeviceId}".WmiQuery();
