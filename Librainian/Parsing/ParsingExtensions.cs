...
//
// File "ParsingExtensions.cs" last formatted on 2022-03-14 at 4:18 AM by Protiguous.
...
public static class ParsingExtensions {
	...
	private const String AllEnglishLetters = ParsingConstants.English.Alphabet.Uppercase + ParsingConstants.English.Alphabet.Lowercase;

	private static readonly Lazy<Regex> WordCounting = new( static () => new( @"[^\ ^\t^\n]+", RegexOptions.Compiled, Seconds.One ) );

	private static Lazy<ParallelQuery<Char>> AllPossibleLetters { get; } = new( static () =>
			 ParallelEnumerable.Range( UInt16.MinValue, UInt16.MaxValue ).AsOrdered().Select( static i => ( Char )i ).Where( Char.IsLetter ) );

	private static Lazy<Regex> ForEnglishOnlyMethod { get; } = new( static () => new( @"(\w+)|(\$\d+\.\d+)", RegexOptions.Compiled, Seconds.One ) );

	private static Lazy<Regex> LowerUnderscore { get; } = new( static () => new( @"([a-z\d])([A-Z])", RegexOptions.Compiled, Seconds.One ) );

	private static Lazy<Regex> NoIdeaToUnderscore { get; } = new( static () => new( @"[-\s]", RegexOptions.Compiled, Seconds.One ) );
	...
	/// <summary>
	///     Create a precompiled <see cref="Regex" /> with a 1 minute timeout.
	/// </summary>
	private static Lazy<Regex> RemoveAllDuplicateLinesRegex { get; } =
		new( static () => new( @"^(.*?)$\s+?^(?=.*^\1$)", RegexOptions.Compiled | RegexOptions.Multiline, Minutes.One ) );

	private static Lazy<Regex> SoundExPart1 { get; } = new( static () => new( @"(\d)\1*D?\1+", RegexOptions.Compiled, Seconds.One ), true );

	private static Lazy<Regex> SoundExPart2 { get; } = new( static () => new( "^D+", RegexOptions.Compiled, Seconds.One ), true );

	private static Lazy<Regex> SoundExPart3 { get; } = new( static () => new( "[D0]", RegexOptions.Compiled, Seconds.One ), true );

	private static Lazy<Regex> UpperToUnderscore { get; } = new( static () => new( @"([A-Z]+)([A-Z][a-z])", RegexOptions.Compiled, Seconds.One ) );

	internal static IAppCache? LazyCache { get; } = new CachingService();

	public static readonly Lazy<Regex> RegexJustNumbers = new( static () => new( "[0-9]", RegexOptions.Compiled | RegexOptions.Singleline, Seconds.One ) );

	static ParsingExtensions() => Debug.Assert( LazyCache is not null );
	...
	public enum CompareOptions {
		...
		IgnoreLeadingWhitespace = 1 << 2,

		IgnoreTrailingWhitespace = 1 << 3,

		IgnoreAllWhitespace = 1 << 4
	}

	/// <summary>
	///     WHY?? For fun. Curiosity.
	/// </summary>
	public static Lazy<String> AllLowercaseLetters { get; } = new( static () => new( AllLetters().Where( Char.IsLower ).Distinct().OrderBy( static c => c ).ToArray() ) );

	/// <summary>
	///     WHY?? For fun. Curiosity.
	/// </summary>
	public static Lazy<String> AllUppercaseLetters { get; } = new( static () => new( AllLetters().Where( Char.IsUpper ).Distinct().OrderBy( static c => c ).ToArray() ) );

	/// <summary>
	///     this doesn't handle apostrophe well
	/// </summary>
	public static Lazy<Regex> RegexBySentenceNotworking { get; } =
		new( static () => new( @"(?<=['""A-Za-z0-9][\.\!\?])\s+(?=[A-Z])", RegexOptions.Compiled | RegexOptions.Multiline, Seconds.One ) );

	public static Lazy<Regex> RegexBySentenceStackoverflow { get; } = new( static () => new( "(?<Sentence>\\S.+?(?<Terminator>[.!?]|\\Z))(?=\\s+|\\Z)",
			 RegexOptions.CultureInvariant | RegexOptions.IgnorePatternWhitespace | RegexOptions.Compiled | RegexOptions.Multiline, Seconds.One ) );

	public static Lazy<Regex> RegexByWordBreak { get; } = new( static () => new( @"(?=\S*(?<=\w))\b", RegexOptions.Compiled | RegexOptions.Singleline, Seconds.One ) );

	public static Lazy<Regex> RegexJustDigits { get; } = new( static () => new( @"\D+", RegexOptions.Compiled | RegexOptions.Singleline, Seconds.One ) );

	public static Char[] SplitBySpace { get; } = {
		ParsingConstants.Spaces.Space
	};
	...
	[NeedsTesting]
	public static String AsOrdinal( this Int32 number ) => ( number % 100 ) switch {
		13 => $"{number}th",
		12 => $"{number}th",
		11 => $"{number}th",
	...
	public static String Capitalize( this String word, Func<String, String> action ) {
		...
		if ( word.IsCapitalized() ) {
			...
			sb.Append( Char.ToUpperInvariant( result[0] ) ).Append( result[1..] );
	...
	public static IDictionary<Char, UInt64> Count( this String text ) {
		...
		text.AsParallel().WithMergeOptions( ParallelMergeOptions.AutoBuffered ).ForAll( c => dict.AddOrUpdate( c, 1, static ( _, arg ) => arg + 1 ) );
	...
	public static String DoubleQuote<T>( this T? self ) {
		...
		if ( ( value?.StartsWith( ParsingConstants.Strings.DoubleQuote, StringComparison.OrdinalIgnoreCase ) == true ) &&
			 value.EndsWith( ParsingConstants.Strings.DoubleQuote, StringComparison.OrdinalIgnoreCase ) ) {
			return value;
		}
	...
	[NeedsTesting]
	public static Word? FirstWordOrDefault( this String sentence ) => sentence.ToWords().FirstOrDefault();
	...
	public static String Format( this Rational rational, Int32 numberOfDigits ) {
		var numeratorShiftedToEnoughDigits = rational.Numerator * BigInteger.Pow( new( 10 ), numberOfDigits );
		var bigInteger = numeratorShiftedToEnoughDigits / rational.Denominator;
		var toBeFormatted = bigInteger.ToString();
		var builder = new StringBuilder();
		builder.Append( toBeFormatted[0] ).Append( '.' ).Append( toBeFormatted[1..numberOfDigits] );
	...
	public static String FullSoundex( this String s ) {
		...
		// do the encoding
		foreach ( var index in s.Select( static t => ParsingConstants.English.Alphabet.Uppercase.IndexOf( t ) ).Where( static index => index >= 0 ) ) {
	...
	/// <summary>
	///     Returns true if any <see cref="Char" /> in the <see cref="String" /> <paramref name="text" /> is a whitespace.
	/// </summary>
	/// <param name="text"></param>
	/// <returns></returns>
	public static Boolean HasWhiteSpace( this String? text ) => text?.Any( Char.IsWhiteSpace ) == true;
	...
	public static Boolean Is( this String? left, String? right ) => String.Equals( left, right, StringComparison.Ordinal );

	public static Boolean IsCapitalized( this String word ) => !String.IsNullOrEmpty( word ) && Char.IsUpper( word, 0 );

	/// <summary>
	///     <para>Returns true if the char is 0,1,2,3,4,5,6,7,8, or 9.</para>
	///     <remarks>
	///         NOTE: <see cref="Char" /> struct already provides an <see cref="Char.IsDigit(Char)" /> method, however it
	///         behaves differently depending on if
	///         char is Latin or not.
	///     </remarks>
	/// </summary>
	/// <param name="c"></param>
	[NeedsTesting]
	...
	public static Boolean IsWhiteSpace( this String? text ) => !String.IsNullOrEmpty( text ) && String.IsNullOrWhiteSpace( text );
	...
	[NeedsTesting]
	public static Word? LastWordOrDefault( this String sentence ) => sentence.ToWords().LastOrDefault();
	...
	/// <returns>String</returns>
	[NeedsTesting]
	public static String MakeInitialLowerCase( this String word, CultureInfo? cultureInfo = null ) => word.Length switch {
		0 => String.Empty,
	...
	public static String OnlyEnglishLetters( this String value!! ) {
		...
		foreach ( var c in value.Where( static c => AllEnglishLetters.Contains( c, StringComparison.CurrentCulture ) ) ) {
			sb.Append( c );
		}
	...
	public static String? OnlyLettersAndNumbers( this String? input ) {
		...
		return String.Concat( input.Where( static c => Char.IsDigit( c ) || Char.IsLetter( c ) ) );
	}
	...
	/// <summary>
	///     Returns all unique rows within a <see cref="String" />.
	/// </summary>
	/// <param name="input"></param>
	/// <returns></returns>
	[NeedsTesting]
	public static String? RemoveAllDuplicateLines( this String? input ) {
		if ( String.IsNullOrEmpty( input ) ) {
			return default( String? );
		}

		return RemoveAllDuplicateLinesRegex.Value.Replace( input, String.Empty );
	}

	/// <summary>
	///     <para>Removes the given string '<paramref name="removeThis" />' from the end of the string.</para>
	/// </summary>
	...
	[NeedsTesting]
	public static String RemoveEnding( this String fromString, String removeThis ) {
		if ( fromString is null ) {
			throw new ArgumentNullException( nameof( fromString ) );
		}

		if ( removeThis is null ) {
			throw new ArgumentNullException( nameof( removeThis ) );
		}

		var index = fromString.LastIndexOf( removeThis, StringComparison.Ordinal );
		if ( index == -1 ) {
			return fromString;
		}

		return fromString.Remove( index );
	}
	...
	public static String ReturnOnlyDigits( this String text ) {
		...
		foreach ( var c in text.Where( static c => c.IsDigit() ) ) {
			sb.Append( c );
		}
	...
	/// </summary>
	/// <param name="s"></param>
	/// <remarks>Faster or slower than <see cref="ReverseAlt" />?</remarks>
	[NeedsTesting]
	...
	/// </summary>
	/// <param name="s"></param>
	/// <remarks>Faster or slowe than <see cref="Reverse" />?</remarks>
	[NeedsTesting]
	public static String ReverseAlt( this String s ) {
		var charArray = s.ToCharArray().ToImmutableArray();

		return new( charArray.Reverse().ToArray() );
	}
	...
	/// <param name="value"></param>
	[NeedsTesting]
	public static String ReverseWords( this String value ) => value.ToWords().Reverse().ToStrings( ParsingConstants.Strings.Singlespace );
	...
	[NeedsTesting]
	public static String ReverseWordsAlt( this String value ) {
		...
		lastIndex = value.Length - position;
	...
	public static IEnumerable<String> SplitToChunks( this String s, Int32 chunks ) {
		...
		var res = Enumerable.Range( 0, s.Length )
							...
							.Select( static g => String.Join( String.Empty, g.Select( static z => z.ch ) ) );
	...
	public static (Status status, String? start) StartsWith( this String value, IEnumerable<String> ofThese, StringComparison comparison = StringComparison.CurrentCulture ) {
		foreach ( var start in ofThese ) {
			if ( value.StartsWith( start, comparison ) ) {
				return (true.ToStatus(), start);
			}
		}

		/*
		foreach ( var start in ofThese.Where( start => value.StartsWith( start, comparison ) ) ) {
			return (true.ToStatus(), start);
		}
		*/
	...
	public static String StripAllWhitespace( this String? value ) {
		...
		foreach ( var c in value.Where( static c => !Char.IsWhiteSpace( c ) ) ) {
			sb.Append( c );
		}
	...
	[NeedsTesting]
	public static IEnumerable<String> ToSplit( this String sentence ) => RegexByWordBreak.Value
																						 .Split(
																							 $"{ParsingConstants.Strings.Singlespace}{sentence}{ParsingConstants.Strings.Singlespace}" )
																						 .ToStrings( ParsingConstants.Spaces.Space )
																						 .Split( SplitBySpace,
																							 StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries );

	/// <summary>
	///     Joins the string <paramref name="list"/>, separated by a single <see cref="Char" />, optionally appending a string.
	/// </summary>
	...
	/// <exception cref="ArgumentEmptyException"></exception>
	[NeedsTesting]
	public static String ToStrings( this IEnumerable<String> list, Char separator, String? atTheEnd = null, Boolean? trimEnd = true ) {
		if ( list is null ) {
	...
	/// <summary>
	///     Joins the strings, separated by a <see cref="String" />, optionally appending a string.
	/// </summary>
	/// <param name="list"></param>
	/// <param name="separator"></param>
	/// <param name="atTheEnd"></param>
	...
	public static String ToStrings( this IEnumerable<Object?> list, String separator, String? atTheEnd = null, Boolean? trimEnd = true ) {
		...
		var joined = String.Join( separator, list.Where( static o => o is not null ) );

		return trimEnd switch {
			true => String.IsNullOrEmpty( atTheEnd ) ? joined.TrimEnd() : $"{joined.TrimEnd()}{separator}{atTheEnd}".TrimEnd(),
			var _ => String.IsNullOrEmpty( atTheEnd ) ? joined : $"{joined}{separator}{atTheEnd}"
		};
	}
	...
	[DebuggerStepThrough]
	[NeedsTesting]
	public static String ToStrings<T>( this IEnumerable<T> list, Char separator = ',', String? atTheEnd = null, Boolean? trimEnd = true ) {
		if ( list is null ) {
			throw new ArgumentEmptyException( nameof( list ) );
		}
	...
	/// <summary>
	///     Returns the given <paramref name="number" /> as english words.
	/// </summary>
	/// <param name="number"></param>
	...
	public static String ToVerbalWord( this Decimal number ) {
		...
		words += " and ";
		words += fraction.ToVerbalWord();

		return words.Trim();
	}

	[NeedsTesting]
	public static IEnumerable<Word> ToWords( this String sentence ) => sentence.ToSplit().Select( static s => new Word( s ) );
	...
	[DebuggerStepThrough]
	[NeedsTesting]
	public static String? Trimmed<T>( this T? self ) => self switch {
		null => default( String? ),
	...
	[DebuggerStepThrough]
	[NeedsTesting]
	public static String? TrimmedAlt<T>( this T? self ) => self switch {
		String s => s.Trim().NullIfEmpty(),
		null => null,
		var _ => self.ToString()?.Trim().NullIfEmpty()
	};
	...
	public static Int32 WordCount( this String input ) {
		...
		try {
			return WordCounting.Value.Matches( input ).Count;
		}
		catch ( Exception ) {
			return -1;
		}
