...
//
// File "Days.cs" last formatted on 2022-12-22 at 5:17 PM by Protiguous.
...
public record Days( BigDecimal Value ) : IQuantityOfTime, IComparable<Days> {
	...
	///     occurs in
	///     the same position in the sort order as <paramref name="other" />. Greater than zero This instance follows
	///     <paramref
	///         name="other" />
	///     in the sort order.
	/// </returns>
	...
	///     in the sort order.
	/// </returns>
	public Int32 CompareTo( IQuantityOfTime? other ) => other is null ? SortingOrder.NullsDefault : this.ToPlanckTimes().CompareTo( other.ToPlanckTimes() );
	...
	public static Boolean operator <=( Days left, Days right ) => left.CompareTo( right ) <= 0;
