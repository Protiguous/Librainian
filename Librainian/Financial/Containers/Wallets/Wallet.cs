...
//
// File "Wallet.cs" last formatted on 2022-02-12 at 7:24 AM by Protiguous.
...
public class Wallet : ABetterClassDispose, IEnumerable<(IDenomination, UInt64)> {
	...
	private async Task<UInt64> Deposit( IBankNote bankNote, UInt64 quantity ) {
		try {
			lock ( this.BankNotes ) {
			...
			var allTimeDeposited = await this.Statistics.GetAllTimeDeposited().ConfigureAwait( false );
			_ = await this.Statistics.SetAllTimeDeposited( allTimeDeposited + ( bankNote.FaceValue * quantity ) );
		}
	}
	...
	[JsonProperty]
	public WalletStatistics Statistics { get; } = new( Seconds.One, CancellationToken.None );
	...
	public void ClearBankNotes() => this.BankNotes.Clear();

	public void ClearCoins() => this.Coins.Clear();
	...
	public async PooledValueTask<UInt64> Deposit( ICoin coin, UInt64 quantity ) {
		try {
			lock ( this.Coins ) {
			...
			var allTimeDeposited = await this.Statistics.GetAllTimeDeposited().ConfigureAwait( false );
			_ = await this.Statistics.SetAllTimeDeposited( allTimeDeposited + ( coin.FaceValue * quantity ) );
		}
	}
	...
	/// <exception cref="WalletException"></exception>
	public async Task<UInt64> Deposit( TransactionMessage message ) =>
		message.Denomination switch {
			IBankNote asBankNote => await this.Deposit( asBankNote, message.Quantity ).ConfigureAwait( false ),
			ICoin asCoin => await this.Deposit( asCoin, message.Quantity ).ConfigureAwait( false ),
	...
	public IEnumerable<ICoin> GetCoins() => this.Coins.SelectMany( static pair => 1.To( pair.Value ), static ( pair, _ ) => pair.Key );
